...
export async function generateService(
    ...
    if (useAI) {
        return await generateWithAI(tableName, tableSchema, servicesDir, spinner, spinnerUpdateInterval);
    } else {
        return generateWithTemplate(tableName, tableSchema, servicesDir, spinner, spinnerUpdateInterval);
    }
}
...
export async function generateWithAI(
    ...
    try {
        ...
        cleanupAndSucceed(spinner, spinnerUpdateInterval, `Service for ${tableName} generated successfully with AI.`);
        return true;
    } catch (error: unknown) {
        ...
        if (err.message.includes("timed out")) {
            ...
            return generateWithTemplate(tableName, tableSchema, servicesDir, spinner, spinnerUpdateInterval, true);
        } else {
            cleanupAndFail(spinner, spinnerUpdateInterval, `Failed to generate service with AI: ${err.message}`);
            return false;
        }
    }
}
...
export function generateWithTemplate(
    ...
    try {
        ...
        cleanupAndSucceed(spinner, spinnerUpdateInterval, `Service for ${tableName} generated successfully using template.`);
        ...
        cleanupAndFail(spinner, spinnerUpdateInterval, `Failed to generate service with template: ${(error as Error).message}`);
        return false;
    }
}
 ...
 */
export async function generateServiceWithAI(tableName: string, tableSchema: Record<string, unknown>): Promise<string> {
    try {

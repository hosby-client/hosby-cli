...
export async function analyzeWithAI(
  ...
  if (!provider) {
    ...
    if (saveProvider) {
      ...
      console.log(
        `✅ ${AI_PROVIDERS[provider as AIProvider].name} saved as your default AI provider`
      );
    }
  }
  ...
  if (!apiKey) {
    throw new Error(
      `No API key provided. Set ${providerConfig.envVarName} environment variable or provide apiKey option.`
    );
  }
  ...
  console.log(
    `📊 Selected ${filterResult.selectedFiles.length} business logic files (${Math.round(filterResult.totalSize / 1024)}KB, ~${filterResult.estimatedTokens} tokens)`
  );

  if (filterResult.estimatedTokens > 120000) {
    console.warn(
      "⚠️ Warning: Project size may exceed token limits. Consider using a smaller project or fewer files."
    );
  }
  ...
  try {
    console.log(
      `🤖 Sending request to ${AI_PROVIDERS[provider as AIProvider].name} model (timeout: ${timeout / 1000}s)...`
    );
    ...
    try {
      ...
      schema = { tables: {} };
      throw new Error(
        "Failed to parse AI response as JSON. The AI may have returned invalid JSON format."
      );
    }
    if (!schema.tables) {
    ...
    if (error.name === "AbortError") {
      throw new Error(
        `AI analysis timed out after ${timeout / 1000} seconds. Try with fewer files or a smaller project.`
      );
    } else if (
      ...
      throw new Error(
        "The project is too large for AI analysis. Try with fewer files or a smaller project."
      );
    } else if (error.code === "rate_limit_exceeded") {
      throw new Error(
        `${AI_PROVIDERS[provider as AIProvider].name} rate limit exceeded. Please try again later or use a different API key.`
      );
    } else if (error.code === "invalid_api_key" || error.message?.includes("invalid api key")) {
      throw new Error(
        `Invalid ${AI_PROVIDERS[provider as AIProvider].name} API key. Please check your API key and try again.`
      );
    } else {

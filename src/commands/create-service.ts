...
export async function createService(tableName?: string): Promise<boolean> {
    try {
        ...
        const useAI = await promptForUsage("useAI", "Would you like to use AI to generate this service?");
        ...
        try {
            ...
            cleanupAndLogError(spinner, spinnerUpdateInterval,
                `Failed to generate service: ${err.message}`, err);
            return false;
        }
    } catch (error: unknown) {
        const err = error as Error;

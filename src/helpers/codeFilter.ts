...
function isBusinessLogicFile(filePath: string): boolean {
    ...
    if (fileName.includes('utils') || fileName.includes('helpers') ||
        fileName.includes('constants') || fileName.includes('config')) {
        return false;
    }
    ...
    const supportedExtensions = [
        ...
        '.vue',
        '.svelte',
        '.component.ts', '.module.ts', '.directive.ts', '.pipe.ts',
        '.astro', '.solid', '.elm'
    ];
    ...
    if (fileName.includes('model') || fileName.includes('schema') ||
        fileName.includes('entity') || fileName.includes('type')) {
        return true;
    }
...
function scoreFileImportance(filePath: string, content: string): number {
    ...
    if (importLines > 0 && codeLines > 0) {
        ...
        if (importRatio > 0.3) { // If more than 30% of lines are imports
            score -= 15;
        }
    }
...
function filterUIComponents(content: string): string {
    ...
    const reactFunctionComponentRegex = /(?:function|const)\s+([A-Z]\w*)(?:\s*:\s*React\.FC(?:<[^>]*>)?)?\s*(?:=\s*(?:\([^)]*\)\s*=>|function\s*\([^)]*\)))?\s*\{[\s\S]*?\}(?=\s*(?:export|;|$))/g;
    ...
    const reactClassComponentRegex = /class\s+([A-Z]\w*)\s+extends\s+(?:React\.)?Component[\s\S]*?\{[\s\S]*?\}(?=\s*(?:export|;|$))/g;
    ...
    const reactHookRegex = /(?:function|const)\s+(use[A-Z]\w*)\s*(?:=\s*(?:\([^)]*\)\s*=>|function\s*\([^)]*\)))?\s*\{[\s\S]*?\}(?=\s*(?:export|;|$))/g;
    filteredContent = filteredContent.replace(reactHookRegex, (match, hookName) => {
        // Check if it's a UI-related hook
        if (isUIComponentName(hookName.substring(3))) { // Remove 'use' prefix for checking
            return '';
        }
        return match;
    });
    ...
    const vueComponentRegex = /(?:@Component|@Options)\s*\([^)]*\)\s*(?:export\s+)?class\s+([A-Z]\w*)[\s\S]*?\{[\s\S]*?\}(?=\s*(?:export|;|$))/g;
    ...
    const vueDefineComponentRegex = /const\s+([A-Z]\w*)\s*=\s*defineComponent\([\s\S]*?\)(?=\s*(?:export|;|$))/g;
    ...
    const angularComponentRegex = /@Component\([\s\S]*?\)\s*(?:export\s+)?class\s+([A-Z]\w*)[\s\S]*?\{[\s\S]*?\}(?=\s*(?:export|;|$))/g;
    ...
    const angularDirectiveRegex = /@Directive\([\s\S]*?\)\s*(?:export\s+)?class\s+([A-Z]\w*)[\s\S]*?\{[\s\S]*?\}(?=\s*(?:export|;|$))/g;
    ...
    const svelteHelperRegex = /(?:export\s+)?class\s+([A-Z]\w*(?:Component|Store|Action))\s*(?:extends\s+\w+)?[\s\S]*?\{[\s\S]*?\}(?=\s*(?:export|;|$))/g;
    ...
    const propsWithModelsRegex = /(?:interface|type)\s+([A-Z]\w*(?:Props|State|Config|Options|Attrs|Events))\s*(?:<[^>]*>)?\s*(?:=\s*)?\{([\s\S]*?)\}(?=\s*(?:export|;|$))/g;
    let propsMatch;
    ...
    while ((propsMatch = propsWithModelsRegex.exec(filteredContent)) !== null) {
        ...
        while ((typeRefMatch = typeRefRegex.exec(propsContent)) !== null) {
            ...
            if (!isUIComponentName(modelType) && !['React', 'Component', 'Element', 'Node', 'JSX'].includes(modelType)) {
    ...
    if (modelTypes.size > 0) {
        console.log(`✔ Extracted ${modelTypes.size} potential model types from UI components: ${Array.from(modelTypes).join(', ')}`);
    }
    ...
    if (modelTypes.size > 0) {
        filteredContent = `// Extracted model types: ${Array.from(modelTypes).join(', ')}\n` + filteredContent;
    }
...
function isUIComponentName(name: string): boolean {
    ...
    const uiPatterns = [
        // Basic UI elements
        'button', 'modal', 'dialog', 'card', 'panel', 'menu', 'nav', 'sidebar',
        'header', 'footer', 'layout', 'container', 'wrapper', 'grid', 'flex',
        'form', 'input', 'select', 'checkbox', 'radio', 'switch', 'toggle',
        'dropdown', 'tooltip', 'popover', 'toast', 'notification', 'alert',
        'spinner', 'loader', 'progress', 'skeleton', 'placeholder', 'icon',
        'avatar', 'badge', 'tag', 'label', 'tab', 'accordion', 'carousel',
        'slider', 'pagination', 'breadcrumb', 'stepper', 'timeline', 'divider',
        'view', 'page', 'screen', 'section', 'row', 'col', 'item', 'list',

        // Animation and transition related
        'animation', 'transition', 'motion', 'animate', 'fade', 'slide', 'zoom',

        // Style and theme related
        'theme', 'style', 'styled', 'css', 'scss', 'sass', 'less',

        // Event handlers and callbacks
        'handler', 'listener', 'callback', 'click', 'hover', 'focus', 'blur',
        'change', 'submit', 'drag', 'drop', 'scroll', 'resize', 'touch',

        // Accessibility related
        'accessible', 'aria', 'a11y',

        // Common UI prefixes/suffixes
        'ui', 'view', 'widget', 'element', 'component', 'control',

        // Framework-specific UI terms
        'hook', 'provider', 'consumer', 'context', 'hoc', 'render',

        // Mobile UI specific
        'swipe', 'gesture', 'touch', 'tap', 'pinch', 'screen',

        // Data visualization but not data models
        'chart', 'graph', 'plot', 'diagram', 'visualization', 'canvas',

        // Media related UI
        'player', 'audio', 'video', 'media', 'stream', 'playback',

        // Common UI utility functions
        'format', 'validate', 'transform', 'convert', 'parse', 'stringify'
    ];
...
export async function filterProjectFiles(
    ...
    const validScoredFiles = scoredFiles
        .filter(file => file !== null) as Array<{ path: string, size: number, content: string, score: number }>;
